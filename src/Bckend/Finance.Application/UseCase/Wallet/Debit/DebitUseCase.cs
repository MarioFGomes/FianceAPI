...
public class DebitUseCase : IDebitUseCase {
    ...
    public DebitUseCase(IWalletRepository walletRepository) {
    ...
    public async Task<ApiResponse<WalletMovimentResponse>> Execute(WalletMovimentRequest request, Guid userId) {
        ...
        try {
            ...
            wallet.Debit(request.Amount, request.Description,request.currency);
            ...
            return new ApiResponse<WalletMovimentResponse> {
                Success = true,
                Message = ResourceMessage.WithdrawalSuccessful,
                Data = new WalletMovimentResponse { Amount = request.Amount, BalanceAfterMovement = wallet.Balance, currency = request.currency,WalletId=wallet.Id,MovimentType= MovimentType.DEBIT, Description = request.Description, CreatedAt = wallet.CreatedAt }
            };
        } catch {
            _walletRepository.RollbackTransaction();
            throw;
        }
    }
