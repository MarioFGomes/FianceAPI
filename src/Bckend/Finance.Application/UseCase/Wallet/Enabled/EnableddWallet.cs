...
public class EnableddWallet: IEnableddWallet {
    private readonly IWalletRepository _walletRepository;
    public EnableddWallet(IWalletRepository walletRepository) {
    ...
    public async Task<bool> Execute(Guid UserId, string currency) {
        ...
        var wallet = await _walletRepository.GetOneIncludingInactiveAsync(e => e.UserId == UserId && e.currency == currency);
        if (wallet is null) throw new InvalidOperationException("A Wallet não foi encontrada");
        wallet.WalletStatus = WalletStatus.Enabled;
